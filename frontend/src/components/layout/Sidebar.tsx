...
export const Sidebar: React.FC = () => {
  ...
  const facilityItems = [
    ...
    { label: 'My Bookings', path: '/app/facilities/bookings/my', icon: CalendarDays },
    { label: 'Add Resource', path: '/app/facilities/resources/add', icon: PlusCircle },
  ];

  const bookingItems = [
    { label: 'Dashboard', path: '/app/bookings/dashboard', icon: LayoutDashboard },
    { label: 'Add Booking', path: '/app/bookings/add', icon: PlusCircle },
    { label: 'My Bookings', path: '/app/bookings/my', icon: CalendarDays },
   
  ];

  if (isAdmin) {
    facilityItems.push({ label: 'Manage Resources', path: '/app/facilities/resources/manage', icon: Settings });
    facilityItems.push({ label: 'Manage Bookings', path: '/app/facilities/bookings/manage', icon: Settings });

    bookingItems.push({ label: 'Manage Bookings', path: '/app/bookings/manage', icon: Settings });
  }
        ...
        <div className="space-y-4">
          ...
          <div>
            <button
              ...
              className={`w-full flex items-center justify-between px-3 py-3 rounded-xl transition-colors ${
                bookingsOpen ? 'bg-slate-800 text-white' : 'text-slate-400 hover:bg-slate-800 hover:text-white'
              }`}
            >
              <div className="flex items-center gap-3 font-semibold text-sm tracking-wide">
                <CalendarDays size={18} className={bookingsOpen ? 'text-blue-500' : ''} />
                Bookings
              </div>
              <ChevronDown size={16} className={`transition-transform duration-300 ${bookingsOpen ? 'rotate-180 text-blue-500' : ''}`} />
            </button>

            <div className={`overflow-hidden transition-all duration-300 ease-in-out ${bookingsOpen ? 'max-h-96 opacity-100' : 'max-h-0 opacity-0'}`}>
              <nav className="pl-5 pr-2 py-2 space-y-1 relative before:absolute before:inset-y-0 before:left-7 before:w-px before:bg-slate-800">
                {renderNavItems(bookingItems)}
              </nav>
            </div>
          </div>
        </div>
